...
//						DBG("gamepad %d, %d", gev->m_gamepad.idx, gev->m_connected);
	...
	bool processEvents(InputState* _input)
	{
		WindowHandle handle = { UINT16_MAX };
		...
		do
		{
			...
			if (NULL != ev)
			{
				...
				case Event::Mouse:
				{
					...
					if (mouse->m_move) {
						if (_input->m_mouse.m_mz == mouse->m_mz) {
							_input->m_mouse.m_mx = mouse->m_mx;
							_input->m_mouse.m_my = mouse->m_my;
							_input->m_mouse.m_mz = mouse->m_mz;
						}
						else {
							_input->m_mouse.m_mx = 0;
				...
				case Event::Key:
				{
					...
					inputSetKeyState(key->m_key, key->m_modifiers, key->m_down);
					_input->m_key.m_buttons[key->m_key] = key->m_down;
				}
				break;
	...
	bool processEvents(MouseState* _mouse)
	{
		WindowHandle handle = { UINT16_MAX };
	...
	bool processEvents(KeyboardState* _keyboard) {
		...
		do
		{
			...
			if (NULL != ev)
			{
				...
				case Event::Key:
				{
					...
					inputSetKeyState(key->m_key, key->m_modifiers, key->m_down);
					_keyboard->m_buttons[key->m_key] = key->m_key;
