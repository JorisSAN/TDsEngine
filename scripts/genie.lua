...
newaction {
	...
	execute = function ()
		...
		generate("temp.bgfx.h" ,      "../externals/bgfx/include/bgfx/c99/bgfx.h", "    ")
		generate("temp.bgfx.idl.inl", "../externals/bgfx/src/bgfx.idl.inl",        "\t")
		generate("temp.defines.h",    "../externals/bgfx/include/bgfx/defines.h",  "\t")
...
newaction {
	trigger = "version",
	description = "Generate TDsEngine version.h",
	execute = function ()
...
--			"Xbox360",
	...
	language "C++"
	startproject "Carousel"

BGFX_DIR     = path.getabsolute("../externals/bgfx")
PROJECTS_DIR = path.getabsolute("../src")
...
function exampleProject(_combined, ...)
	...
	if _combined then
		...
		for _, name in ipairs({...}) do
			...
			files {
				path.join(PROJECTS_DIR, name, "**.c"),
				path.join(PROJECTS_DIR, name, "**.cpp"),
				path.join(PROJECTS_DIR, name, "**.h"),
			}

			removefiles {
				path.join(PROJECTS_DIR, name, "**.bin.h"),
			}
...
dofile "../externals/bgfx/scripts/bgfx.lua"
...
if _OPTIONS["with-tools"] then
	group "tools"
	dofile "../externals/bgfx/scripts/shaderc.lua"
	dofile "../externals/bgfx/scripts/texturec.lua"
	dofile "../externals/bgfx/scripts/texturev.lua"
	dofile "../externals/bgfx/scripts/geometryc.lua"
	dofile "../externals/bgfx/scripts/geometryv.lua"
end
